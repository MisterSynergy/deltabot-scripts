...
while True:
    ...
    for m in data["query"]["allpages"]:
        try:
            property = m["title"][9:]
            r2 = requests.get(
                "https://plnode.toolforge.org/triplecount?query=SELECT ?a WHERE {{?a p:{:s} ?o}}".format(
                    property
                )
            )
            data2 = r2.json()
            ...
            r2 = requests.get(
                "https://plnode.toolforge.org/triplecount?query=PREFIX pq: <http://www.wikidata.org/prop/qualifier/> SELECT ?a WHERE {{?a pq:{:s} ?o}}".format(
                    property
                )
            )
            data2 = r2.json()
            ...
            r2 = requests.get(
                "https://plnode.toolforge.org/triplecount?query=PREFIX pr: <http://www.wikidata.org/prop/reference/> SELECT ?a WHERE {{?a pr:{:s} ?o}}".format(
                    property
                )
            )
            data2 = r2.json()
...
text += "}}</includeonly>\n"
text += "<noinclude>{{Documentation}}</noinclude>"
...
header = 'A list of the top 100 [[Help:Properties|properties]] by quantity of item pages that link to them. Data as of <onlyinclude>{0}</onlyinclude>.\n\n{{| class="wikitable sortable plainlinks" style="width:100%%; margin:auto;"\n|-\n! Property !! Quantity of item pages\n'
table_row = "|-\n| {{{{P|{0}}}}} || [//wikidata.org/wiki/Special:WhatLinksHere/Property:{0}?namespace=0 {1}]\n"
...
page = pywikibot.Page(site, "Wikidata:Database reports/List of properties/Top100")
page.put(text, summary="upd", minorEdit=False)
